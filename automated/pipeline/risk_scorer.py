def score_risk(metadata_fields: list[str], pii_results: list[dict]) -> int:
    ...
    metadata_lower = [str(f).lower() for f in metadata_fields]
    ...
    if any(any(kw in f for kw in ["author", "user", "creator", "owner", "name"]) for f in metadata_lower):
        score += 25
        
    # Device serial / IMEI check
    if any(any(kw in f for kw in ["serial", "imei", "device id", "uuid"]) for f in metadata_lower):
        score += 20
    ...
    if any(any(kw in f for kw in ["software", "version", "program", "creator tool"]) for f in metadata_lower):
        score += 7
        
    # Timestamps check
    if any(any(kw in f for kw in ["time", "date", "created"]) for f in metadata_lower):
        score += 5
